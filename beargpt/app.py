...
import os, time
from assistant import openai, chat_history, config
...
def flush():
    chat_history.flush_chat_history(session['default_chat_session_id'])
...
def chats_post(session_id):
    ...
    return render_template("index.html", chat_history=history, chat_sessions=chat_sessions, streaming=stream, session_id=session_id)
